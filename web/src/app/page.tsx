...
export default function Home() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      // Usar una URL de ejemplo para pruebas en modo local
      const testUrl = "https://www.example.com";
      const urlToProcess = url || testUrl;
      
      console.log("Enviando solicitud al backend...", urlToProcess);

      // Avanzar al siguiente paso de progreso para mostrar que estamos procesando
      ...
      // Avanzar al siguiente paso de progreso
      ...
      alert(`Error: ${error.message || 'Ocurrió un problema al procesar la solicitud'}`);
      setIsLoading(false);
      setStep(2);
    }
  };
        ...
        <form onSubmit={handleNext} className="max-w-2xl mx-auto mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <button
              type="submit"
              disabled={!url.trim()}
              className={`px-6 py-3 rounded-lg text-white font-medium transition-colors ${!url.trim() ? 'bg-gray-400' : 'bg-primary hover:bg-primary/90'}`}
            >
              Continuar
            </button>
          </div>
