...
try {
  ...
  console.log(
    servicesStatus.fal
      ? "✅ fal.ai configurado correctamente"
      : "⚠️  No hay clave de fal.ai configurada",
  );
} catch (error) {
...
servicesStatus.n8n =
  !!process.env.N8N_MCP_PROD_URL &&
  !process.env.N8N_MCP_PROD_URL.includes("example");
console.log(
  servicesStatus.n8n
    ? "✅ n8n configurado correctamente"
    : "⚠️  No hay URL válida para n8n configurada",
);
...
const errorHandler = (
  ...
  res
    .status(500)
    .json({
      error: MODE === "local" ? err.message : "Error interno del servidor",
    });
};
...
app.use(
  cors({
    origin: true, // Permitir todas las solicitudes en desarrollo
    credentials: true,
...
const limiter = rateLimit({
  ...
  max: parseInt(
    process.env.RATE_LIMIT_MAX || (MODE === "local" ? "1000" : "100"),
    10,
  ), // Más permisivo en local
...
✈️ Procesando URL: ${url}`);
    ...
    try {
      ...
      generatedContent = await generateContent(scrapedContent, prompts);
      console.log("✅ Contenido generado correctamente");
    } catch (aiError: any) {
      console.error(
        "❌ Error al generar contenido con OpenAI:",
        aiError.message,
      );
    ...
    try {
      console.log(
        "3⃣ Generando imagen con fal.ai:",
        generatedContent.image_prompt,
      );
      const imageResult = await generateImage(
        generatedContent.image_prompt,
        (message) => console.log(`[fal.ai] ${message}`)
      );
      console.log("✅ Imagen generada correctamente:", imageResult.imageUrl);

      // Añadir URL de la imagen al resultado
      generatedContent.article_html = generatedContent.article_html.replace(
        "</h1>",
        `</h1><img src="${imageResult.imageUrl}" alt="Imagen generada" class="w-full h-auto rounded-lg my-4" />`,
      );
      generatedContent.image_url = imageResult.imageUrl;
    } catch (imageError: any) {
      console.warn(
        "⚠️ Error al generar imagen, continuando sin ella:",
        imageError.message,
      );
      ...
      generatedContent.article_html = generatedContent.article_html.replace(
        "</h1>",
        `</h1><img src="${fallbackImageUrl}" alt="Imagen de prueba" class="w-full h-auto rounded-lg my-4" />`,
      );
...
app.get("/api/health", (req, res) => {
  ...
  res.json({
    ...
    env: {
      n8n: process.env.N8N_MCP_PROD_URL
        ? process.env.N8N_MCP_PROD_URL !== "" &&
          !process.env.N8N_MCP_PROD_URL.includes("example")
          ? "configured"
          : "invalid"
